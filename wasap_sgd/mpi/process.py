...
class MPIProcess(object):
    ...
    def __init__(
        ...
        self.time_step = 0
        self._is_shadow = (
            self.process_comm is not None and self.process_comm.Get_rank() != 0
        )
        ...
        if (self.parent_rank is not None and self.parent_comm is not None) or (
            self.process_comm
        ):
            self.train()
    ...
    def is_shadow(self, sync=False):
        ...
        if self.process_comm and sync:
            ...
            self.logger.debug(
                "syncing on the process communicator from", inspect.stack()[1][3]
            )
            self.process_comm.Barrier()
    ...
    def check_sanity(self):
        ...
        for par in ["model", "update"]:
            if not hasattr(self, par) or getattr(self, par) is None:
                raise Error(
                    "%s not found!  Process %s does not seem to be set up correctly."
                    % (par, self.ranks)
                )
    ...
    def lookup_mpi_tag(self, name, inv=False):
        ...
        try:
            ...
            self.logger.error(
                "Not found in tag dictionary: {0} -- returning None".format(name)
            )
            return None
    ...
    def recv(self, tag=MPI.ANY_TAG, source=None, status=None, comm=None):
        ...
        if source is None:
            if self.parent_rank is None:
                raise Error(
                    "Attempting to receive %s from parent, but parent rank is None"
                    % tag
                )
    ...
    def send(self, obj, tag, dest=None, comm=None):
        ...
        if dest is None:
            if self.parent_rank is None:
                raise Error(
                    "Attempting to send %s to parent, but parent rank is None" % tag
                )
    ...
    def send_arrays(
        self, obj, expect_tag, tag, comm=None, dest=None, check_permission=False
    ):
    ...
    def send_final_weights(self, comm=None, dest=None, check_permission=False):
        ...
        self.send_arrays(
            self.model.get_weights(),
            expect_tag="begin_final_weights",
            tag="final_weights",
            comm=comm,
            dest=dest,
            check_permission=check_permission,
        ...
        if add_to_existing:
            ...
            for index, v in tmp.items():
                ...
                if index not in self.update:
                    ...
                    self.update[index] = (
                        self.update[index][0] + dw,
                        self.update[index][1] + delta,
                    )
...
class MPIWorker(MPIProcess):
    ...
    def __init__(
        ...
        if tell_comm:
            logging.info(
                info.format(tell_comm.Get_rank(), parent_rank, tell_comm.Get_size())
            )
    ...
    def train(self, testing=False):
        ...
        for x_b, y_b in self.data.generate_data():
            ...
            if num_batches % batches_per_epoch == 0:
                ...
                if epoch >= self.num_epochs:
                    ...
                    accuracy_test, activations_test = self.model.predict(
                        self.data.get_test_data(), self.data.get_test_labels()
                    )
                    t4 = datetime.datetime.now()
                    loss_test = self.model.compute_loss(
                        self.data.get_test_labels(), activations_test
                    )
                ...
                self.logger.info(
                    f"Finishing epoch {epoch}, now validate every is {self.algo.sync_every}"
                )
        ...
        if self.monitor and self.save_filename != "":
            with open(self.save_filename + "_monitor.json", "w") as file:
                file.write(
                    json.dumps(
                        self.monitor.get_stats(), indent=4, sort_keys=True, default=str
                    )
                )
...
class MPIMaster(MPIProcess):
    ...
    def __init__(
        ...
        self.num_workers = (
            child_comm.Get_size() - 1
        )  # all processes but one are workers
        self.metrics = np.zeros((num_epochs + 1, 4))

        self.num_sync_workers = num_sync_workers
        info = (
            "Creating MPIMaster with rank {0} and parent rank {1}. "
            "(Communicator size {2}, Child communicator size {3})"
        )
        logging.info(
            info.format(
                parent_comm.Get_rank(),
                parent_rank,
                parent_comm.Get_size(),
                child_comm.Get_size(),
            )
        )
        if self.num_sync_workers > 1:
            logging.info(
                "Will wait for updates from {0:d} workers before synchronizing".format(
                    self.num_sync_workers
                )
            )

        super(MPIMaster, self).__init__(
            parent_comm,
            process_comm=None,
    ...
    def do_average_sequence(self, source):
        ...
        if len(self.candidates) == self.num_workers:
            ...
            for i, weight in self.new_weights["w"].items():
                ...
                n_w_to_delete = (
                    self.new_weights["w"][i].count_nonzero()
                    - self.weights["w"][i].count_nonzero()
                )
                ...
                self.new_weights["w"][i] = coo_matrix(
                    (vals_w_new, (rows_w_new, cols_w_new)), (weight.shape)
                ).tocsr()
    ...
    def do_update_sequence(self, source):
        ...
        if accepted:
            ...
            if self.decide_whether_to_sync():
                ...
                self.apply_update(
                    sync=self.is_synchronous(), retain=retain, worker=False
                )
                    ...
                    if self.time_step % self.algo.validate_every == 0:
                        ...
                        if self.epoch < self.num_epochs - 1:
                            ...
                            if not self.is_synchronous():
                                self.evolution_workers_list = list(
                                    range(1, self.num_workers + 1)
                                )
                                self.evolution_workers_list.remove(source)
                        ...
                        self.logger.info(
                            f"Master epoch {self.epoch}, timestep {self.time_step}"
                        )
                        self.logger.info(f"Learning rate is {self.model.learning_rate}")
    ...
    def validate(self):
        ...
        accuracy_train, activations_train = self.model.predict(
            self.data.get_train_data(), self.data.get_train_labels()
        )
        t4 = datetime.datetime.now()
        ...
        loss_train = self.model.compute_loss(
            self.data.get_train_labels(), activations_train
        )
