...
def get_groups(comm, num_masters=1, num_processes=1):
    masters = list(
        range(0, num_masters)
    )  # index 0 is the supervisor master, the other one are sub-masters
    ...
    for ir, i_rank in enumerate(instances_ranks):
        ...
        processes[ir] = list(
            range(i_rank, min(i_rank + num_processes, comm.Get_size()))
        )
...
class MPIManager(object):
    ...
    def __init__(
        ...
        if comm.Get_size() != 1:
            n_instances, remainder = divmod(
                comm.Get_size() - self.num_masters, self.num_processes
            )
        else:
            n_instances, remainder = 1, 0
        if remainder:
            logging.info(
                "The accounting is not correct, %d nodes are left behind", remainder
            )
    ...
    def make_comms(self, comm):
        ...
        rank = comm.Get_rank()
        masters, groups, processes = get_groups(
            comm, self.num_masters, self.num_processes
        )
        ...
        logging.debug(
            "master comm", self.comm_masters.Get_size() if self.comm_masters else "N/A"
        )
        logging.debug(
            "block comm", self.comm_block.Get_size() if self.comm_block else "N/A"
        )
        logging.debug(
            "instance comm",
            self.comm_instance.Get_size() if self.comm_instance else "N/A",
        )
