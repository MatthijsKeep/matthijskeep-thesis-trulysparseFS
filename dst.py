...
class dst_FS:
        ...
        for name, param in self.model.named_parameters():
            if param.requires_grad:
                if len(param.shape) > 1:
                    if (
                        idx == 1 and self.hidden_IMP == False
                    ):  ### neurons of hidden layer is equally important
                        self.layers_importance[str(idx)] = torch.ones(
                            param.shape[1]
                        ).to(self.device)
                    else:
                        self.layers_importance[str(idx)] = torch.zeros(
                            param.shape[1]
                        ).to(self.device)
        ...
        for name, param in self.model.named_parameters():
            if param.requires_grad:
                if len(param.shape) > 1:
                    ...
                    self.mask[name] = (
                        (torch.rand(param.data.shape) < self.density)
                        .float()
                        .data.to(self.device)
                    )
                    ...
                    print(
                        " OutDegreeBottomNeurons %.2f ± %.2f; InDegreeTopNeurons %.2f ± %.2f"
                        % (
                            torch.mean(self.mask[name].sum(axis=1)),
                            torch.std(self.mask[name].sum(axis=1)),
                            torch.mean(self.mask[name].sum(axis=0)),
                            torch.std(self.mask[name].sum(axis=0)),
                        )
                    )
    ...
    # Drop. connection importance is based on its magnitude (QS baseline)
                    ...
                    smallestPositive = values[
                        int(
                            min(
                                values.shape[0] - 1,
                                lastZeroPos
                                + self.prune_rate * (values.shape[0] - lastZeroPos),
                            )
                        )
                    ]
    ...
    # Drop. connection importance is based on its magnitude and importance of connected neuron (WAST)
    def rmv_IMP(self):
        idx = 0
        self.num_weights = {}
        for name, param in self.model.named_parameters():
            if param.requires_grad:
                if len(param.shape) > 1:
                    ...
                    if idx == 0:
                        weights = (
                            param * self.mask[name]
                        ).detach() * self.layers_importance[str(idx)]
                    else:
        ...
        for name, param in self.model.named_parameters():
            if param.requires_grad:
                if len(param.shape) > 1:
                    ...
                    new_conn_idx = np.random.choice(
                        range(idx_zeros_i.shape[0]),
                        size=int(noRewires.data),
                        replace=False,
                    )
                    self.mask[name][
                        idx_zeros_i[new_conn_idx], idx_zeros_j[new_conn_idx]
                    ] = 1
    ...
    def add_IMP(self):
        ...
        # NOTE: (M): Looks like this is WAST
        idx = 0
        for name, param in self.model.named_parameters():
            if param.requires_grad:
                if len(param.shape) > 1:
                    ...
                    layer_importance = torch.mm(
                        self.layers_importance[nxt_layer_name].reshape(
                            self.layers_importance[nxt_layer_name].shape[0], 1
                        ),
                        self.layers_importance[layer_name]
                        .reshape(self.layers_importance[layer_name].shape[0], 1)
                        .T,
                    )
                    layer_importance[self.mask[name] == 1] = -self.inf
                    layer_importance = -layer_importance.flatten()
                    idx_add = np.argpartition(
                        layer_importance.detach().cpu(), noRewires
                    )
