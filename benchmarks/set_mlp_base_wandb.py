...
class SET_MLP:
    def __init__(self, dimensions, activations, epsilon=20, weight_init='he_uniform', config=None):
        """
...
if __name__ == "__main__":
    ...
    for i in range(runs):
        ...
        set_mlp = SET_MLP((x_train.shape[1], no_hidden_neurons_layer, no_hidden_neurons_layer, no_hidden_neurons_layer, y_train.shape[1]),
                          (AlternatedLeftReLU(-0.6), AlternatedLeftReLU(0.6), AlternatedLeftReLU(-0.6), Softmax), epsilon=epsilon, config=config)
