...
def load_mnist_data(n_training_samples, n_testing_samples):
    ...
    # # convert 1 hot to label
    # y_train = np.argmax(y_train, axis=1)
    # y_test = np.argmax(y_test, axis=1)

    x_train, x_test = x_train.reshape(x_train.shape[0], 784), x_test.reshape(
        x_test.shape[0], 784
    )
...
def load_har():
    ...
    print(
        f"Shapes are: X_train: {X_train.shape}, y_train: {y_train.shape}, X_test: {X_test.shape}, y_test: {y_test.shape}"
    )
...
def load_synthetic(
    ...
    print(
        f"The informative features are the first {n_informative+n_redundant} features"
    )
    ...
    print(
        f"Expected train shape: {(n_samples*0.8, n_features)}, actual train shape: {train_X.shape}"
    )
    print(
        f"Expected test shape: {(n_samples*0.1, n_features)}, actual test shape: {test_X.shape}"
    )
    print(
        f"Expected val shape: {(n_samples*0.1, n_features)}, actual val shape: {val_X.shape}"
    )

    print(
        f"shapes are: {train_X.shape}, {train_y.shape}, {test_X.shape}, {test_y.shape}"
    )
