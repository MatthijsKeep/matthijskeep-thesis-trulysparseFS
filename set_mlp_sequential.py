...
class SET_MLP:
    ...
    def _input_pruning(self, epoch, i):
        ...
        if zerow_before > self.input_sum.shape[0] - (args.K * 2):
            print(
                f"WARNING: No more neurons to prune, since {zerow_before} > {self.input_sum.shape[0] - (args.K * 2)}"
            )
    ...
    def fit(
        ...
        for i in range(epochs):
            ...
            if (
                ...
                importances_path = (
                    f"importances/importances_{str(args.data)}_{i}_MLP.csv"
                )
            ...
            if testing:
                ...
                if i % 2 == 0:
                    selected_features, importances = select_input_neurons(
                        copy.deepcopy(self.w[1]), args.K
                    )
                    accuracy_topk, pct_correct = evaluate_fs(
                        x, x_test, y_true, y_test, selected_features
                    )
                    wandb.log(
                        {
            ...
            if (
                i < epochs - 1 and not args.update_batch
    ...
    def _plot_loss_from_metrics(self, metrics, run):
        ...
        for i in range(run + 1):
            print(
                f"Amount of nan values in run {i}: {np.count_nonzero(np.isnan(metrics[i, :, 0]))}"
            )
            # print(metrics[i, :, :])
    ...
    def _plot_input_distribution(self, epoch, values):
        ...
        plt.title(
            f"Distribution of the sum of weights of the input layer in epoch {str(epoch)}"
        )
        # save in a folder called "input_weight_distribution"
        plt.savefig(f"input_weight_distribution/epoch_{str(epoch)}.png")
        plt.close()
        print(
            f"Plotting the input weight distribution took {time.time() - plot_time_start} seconds."
        )
...
if __name__ == "__main__":
    ...
    zeta = (
        args.zeta
    )  # in [0..1]. It gives the percentage of unimportant connections which are removed and replaced with random ones after every epoch
    ...
    for i in range(runs):
        ...
        # convert to csv
        selected_features_for_eval = selected_features_for_eval.to_csv(
            header=False, index=False
        )

        selected_features_path = (
            f"features/selected_features_{str(args.data)}_{no_training_epochs}_MLP.csv"
        )
        ...
        importances_path = (
            f"importances/importances_{str(args.data)}_{no_training_epochs}_MLP.csv"
        )
        if not os.path.exists(os.path.dirname(importances_path)):
            os.makedirs(os.path.dirname(importances_path))
        ...
        start_time = time.time()
        print(
            f"Accuracy of the last epoch on the testing data (with {args.K} features): ",
            accuracy_topk,
        )
        accuracies.append(accuracy_topk)
        print_and_log(accuracy_topk)
    np.savetxt(
        f"benchmarks/results/truly_sparse_FS/{args.data}_{runs}.csv",
        accuracies,
        delimiter=",",
    )
    print(accuracies)
    print(
        f"Average training time over {runs} runs is {sum_training_time/runs} seconds, with mean accuracy {round(np.mean(accuracies)*100, 4)}% and std {round(np.std(accuracies)*100, 4)}%"
    )
