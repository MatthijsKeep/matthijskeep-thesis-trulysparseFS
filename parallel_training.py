...
def shared_partitions(n, num_workers, batch_size):
    ...
    for w in range(num_workers):
        data[w] = dinds[
            w * batch_size * worker_size : (w + 1) * batch_size * worker_size
        ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--monitor",
        help="Monitor cpu and gpu utilization",
        default=False,
        action="store_true",
    )
    ...
    parser.add_argument(
        "--masters", help="number of master processes", default=1, type=int
    )
    parser.add_argument(
        "--processes", help="number of processes per worker", default=1, type=int
    )
    parser.add_argument(
        "--synchronous", help="run in synchronous mode", action="store_true"
    )
    ...
    parser.add_argument(
        "--sync-every",
        help="how often to sync weights with master",
        default=1,
        type=int,
        dest="sync_every",
    )
    ...
    parser.add_argument(
        "--dropout-rate", type=float, default=0.3, help="Dropout rate (default: 0.3)"
    )
    parser.add_argument(
        "--weight-decay",
        type=float,
    ...
    parser.add_argument(
        "--epsilon", type=int, default=20, help="Sparsity level (default: 20)"
    )
    parser.add_argument(
        "--zeta",
        type=float,
    ...
    parser.add_argument(
        "--prune", default=True, help="Perform Importance Pruning", action="store_true"
    )
    ...
    parser.add_argument(
        "--n-testing-samples", type=int, default=10000, help="Number of testing samples"
    )
    parser.add_argument(
        "--augmentation", default=True, help="Data augmentation", action="store_true"
    )
    parser.add_argument(
        "--dataset",
        default="fashionmnist",
        help='Specify dataset. One of "cifar10", "fashionmnist",'
        '"madelon",  or "mnist"',
    )
    ...
    if args.dataset == "fashionmnist" or args.dataset == "mnist":
        ...
        if args.dataset == "fashionmnist":
            X_train, Y_train, X_test, Y_test = load_fashion_mnist_data(
                args.n_training_samples, args.n_testing_samples
            )
        else:
        ...
        loss = "cross_entropy"
        activations = (
            AlternatedLeftReLU(-0.75),
            AlternatedLeftReLU(0.75),
            AlternatedLeftReLU(-0.75),
            Softmax,
        )
        if args.augmentation:
            X_train, Y_train, X_test, Y_test = load_cifar10_data_not_flattened(
                args.n_training_samples, args.n_testing_samples
            )
        else:
    ...
    initialize_logger(
        filename=log_file, file_level=args.log_level, stream_level=args.log_level
    )
    ...
    if num_processes == 1:
        ...
        if rank != 0:
            ...
            data = Data(
                batch_size=batch_size,
                x_train=X_train[partitions[rank - 1]],
    ...
    if rank == 0:
        ...
        logging.info(
            "Testing time is {0:.3f} seconds".format(manager.process.validate_time)
        )
        delta_t -= manager.process.validate_time
        logging.info("Training finished in {0:.3f} seconds".format(delta_t))
        logging.info(
            "Evolution time is {0:.3f} seconds".format(manager.process.evolution_time)
        )
