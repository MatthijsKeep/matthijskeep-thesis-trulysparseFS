...
def get_loaders(train, test, batch_size, test_batch_size):
    train_loader = torch.utils.data.DataLoader(
        train, batch_size, num_workers=8, pin_memory=True, shuffle=True
    )

    print("Train loader length", len(train_loader))
    valid_loader = None
    test_loader = torch.utils.data.DataLoader(
        test, test_batch_size, shuffle=False, num_workers=1, pin_memory=True
    )
...
def get_dataset_loader(args):
    if args.data == "mnist":
        ...
        train_loader, valid_loader, test_loader = get_FashionMnist_dataloaders(
            args, validation_split=args.valid_split
        )
        input_size = 28 * 28
        output_size = 10
    elif args.data == "madelon":
        train_loader, valid_loader, test_loader = get_madelon_dataloaders(
            args, validation_split=args.valid_split
        )
        input_size = 500
        output_size = 2
    elif args.data == "coil":
        train_loader, valid_loader, test_loader = get_coil_dataloaders(
            args, validation_split=args.valid_split
        )
        input_size = 1024
        output_size = 20
    elif args.data == "USPS":
        train_loader, valid_loader, test_loader = get_USPS_dataloaders(
            args, validation_split=args.valid_split
        )
        input_size = 256
        output_size = 10
    elif args.data == "HAR":
        train_loader, valid_loader, test_loader = get_HAR_dataloaders(
            args, validation_split=args.valid_split
        )
        input_size = 561
        output_size = 6
    elif args.data == "Isolet":
        train_loader, valid_loader, test_loader = get_Isolet_dataloaders(
            args, validation_split=args.valid_split
        )
        input_size = 617
        output_size = 26
    elif args.data == "PCMAC":
        train_loader, valid_loader, test_loader = get_PCMAC_dataloaders(
            args, validation_split=args.valid_split
        )
        input_size = 3289
        output_size = 2
    elif args.data == "SMK":
        train_loader, valid_loader, test_loader = get_SMK_dataloaders(
            args, validation_split=args.valid_split
        )
        input_size = 19993
        output_size = 2
    elif args.data == "GLA":
        train_loader, valid_loader, test_loader = get_GLA_dataloaders(
            args, validation_split=args.valid_split
        )
        input_size = 49151
        output_size = 4
...
def get_madelon_dataloaders(args, validation_split=0.0):
    ...
    train_loader = torch.utils.data.DataLoader(
        train, args.batch_size, num_workers=8, pin_memory=True, shuffle=True
    )

    print("Train loader length", len(train_loader))
    valid_loader = None
    test_loader = torch.utils.data.DataLoader(
        test, args.test_batch_size, shuffle=False, num_workers=1, pin_memory=True
    )
    return train_loader, valid_loader, test_loader
...
def get_coil_dataloaders(args, validation_split=0.0):
    ...
    transform = transforms.Compose([transforms.ToTensor(), normalize])
    train = custom_data(
        "coil", "./data", train=True, download=True, transform=transform
    )
    test = custom_data(
        "coil", "./data", train=False, download=True, transform=transform
    )
    train_loader, valid_loader, test_loader = get_loaders(
        train, test, args.batch_size, args.test_batch_size
    )
    return train_loader, valid_loader, test_loader
...
def get_USPS_dataloaders(args, validation_split=0.0):
    ...
    transform = transforms.Compose([transforms.ToTensor(), normalize])
    train = custom_data(
        "USPS", "./data", train=True, download=True, transform=transform
    )
    test = custom_data(
        "USPS", "./data", train=False, download=True, transform=transform
    )
    train_loader, valid_loader, test_loader = get_loaders(
        train, test, args.batch_size, args.test_batch_size
    )
...
def get_PCMAC_dataloaders(args, validation_split=0.0):
    ...
    transform = transforms.Compose([transforms.ToTensor(), normalize])
    train = custom_data(
        "PCMAC", "./data", train=True, download=True, transform=transform
    )
    test = custom_data(
        "PCMAC", "./data", train=False, download=True, transform=transform
    )
    train_loader, valid_loader, test_loader = get_loaders(
        train, test, args.batch_size, args.test_batch_size
    )
...
def get_Isolet_dataloaders(args, validation_split=0.0):
    ...
    transform = transforms.Compose([transforms.ToTensor(), normalize])
    train = custom_data(
        "Isolet", "./data", train=True, download=True, transform=transform
    )
    test = custom_data(
        "Isolet", "./data", train=False, download=True, transform=transform
    )
    train_loader, valid_loader, test_loader = get_loaders(
        train, test, args.batch_size, args.test_batch_size
    )
...
def read_PCMAC():
    ...
    y = mat["Y"]
    train_X, test_X, train_y, test_y = train_test_split(
        X, y, test_size=0.20, ranm_state=42
    )
    return train_X, train_y, test_X, test_y
